...
func newIngressTemplate(serviceNameForBackend string) networkingv1.IngressSpec {
	...
	return ingresses.NewIngressTemplate(ingressHostName, paths)
}

// newServiceTemplate is a private constructor that returns service spec for specific workloads
func newServiceTemplate(selector map[string]string) corev1.ServiceSpec {
	...
	return services.NewServiceTemplate(serviceType, ports, selector)
}
...
func waitUntilIngressHostnameUpdates(client *rancher.Client, clusterID, namespace, ingressName string) error {
	...
	adminIngressResource := adminDynamicClient.Resource(ingresses.IngressesGroupVersionResource).Namespace(namespace)
...
func validateDaemonset(t *testing.T, client *rancher.Client, clusterID, namespaceName, daemonsetName string) {
	...
	daemonset, err := daemonsets.GetDaemonsetByName(client, clusterID, namespaceName, daemonsetName)
	require.NoError(t, err)

	assert.Equalf(t, int(daemonset.Status.NumberAvailable), len(workerNodesCollection.Data), "Daemonset %v doesn't have the required ready", daemonsetName)
}
