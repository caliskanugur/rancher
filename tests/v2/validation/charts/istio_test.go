...
func (i *IstioTestSuite) TestUpgradeIstioChart() {
	...
	require.NoError(i.T(), err)
	assert.Equalf(i.T(), 2, len(deploymentListPreUpgrade.Items), "Pilot & Ingressgateways deployments don't have the correct istio version labels")
	...
	for _, deployment := range deploymentListPreUpgrade.Items {
		...
		assert.Equalf(i.T(), istioVersionPreUpgrade, imageVersion, "Pilot & Ingressgateways images don't use the correct istio image version")
	}
	...
	require.NoError(i.T(), err)
	assert.Equalf(i.T(), 2, len(deploymentListPostUpgrade.Items), "Pilot & Ingressgateways deployments don't have the correct istio version labels")
	...
	for _, deployment := range deploymentListPostUpgrade.Items {
		...
		assert.Equalf(i.T(), istioVersionPostUpgrade, imageVersion, "Pilot & Ingressgateways images don't use the correct istio image version")
	}
}
