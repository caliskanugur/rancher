...
const (
	...
	ConstraintType             = "constraints.gatekeeper.sh.k8srequiredlabels"
)
...
func getAuditTimestamp(client *rancher.Client, project *management.Project) error {
	...
	steveClient, err := client.Steve.ProxyDownstream(project.ClusterID) //ProxyClusterClient(project.ClusterID)
	if err != nil {
		return err
	}
	return wait.Poll(1*time.Second, 5*time.Minute, func() (done bool, err error) {
		...
		auditList, err := steveClient.SteveType(ConstraintType).List(&types.ListOpts{})
		if err != nil {
			return false, nil
		}
