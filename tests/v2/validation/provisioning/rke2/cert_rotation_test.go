...
func (r *V2ProvCertRotationTestSuite) testCertRotation(provider Provider, kubeVersion string, nodesAndRoles []machinepools.NodeRoles, credential *cloudcredentials.CloudCredential) {
	...
	r.Run(name, func() {
		r.Run("initial", func() {
			...
			steveCluster, err := r.client.Steve.SteveType(clusters.ProvisioningResouceType).ByID(clusterResp.ID)
			require.NoError(r.T(), err)
...
func (r *V2ProvCertRotationTestSuite) rotateCerts(id string, generation int64) error {
	...
	cluster, err := r.client.Steve.SteveType(clusters.ProvisioningResouceType).ByID(id)
	if err != nil {
		return err
	}

	spec := &apiv1.ClusterSpec{}
	clusterSpec, err := v1.ConvertSpecOrStatusType(cluster.Spec, spec)
	require.NoError(r.T(), err)

	updatedCluster := *cluster

	clusterSpec.(*apiv1.ClusterSpec).RKEConfig.RotateCertificates = &rkev1.RotateCertificates{
		Generation: generation,
	}

	updatedCluster.Spec = *clusterSpec.(*apiv1.ClusterSpec)

	cluster, err = r.client.Steve.SteveType(clusters.ProvisioningResouceType).Update(cluster, updatedCluster)
	if err != nil {
		return err
	}
