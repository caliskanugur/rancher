...
const (
	ProvisioningResouceType = "provisioning.cattle.io.cluster"

	clusterStateUpdating        = "updating"
	clusterErrorStateMessage    = "cluster is in error state"
	clusterCPErrorMessage       = "Cluster health check failed: Failed to communicate with API server during namespace check"
...
func CreateK3SRKE2Cluster(client *rancher.Client, rke2Cluster *apisV1.Cluster) (*v1.SteveAPIObject, error) {
	cluster, err := client.Steve.SteveType(ProvisioningResouceType).Create(rke2Cluster)
	if err != nil {
		return nil, err
	}
	...
	client.Session.RegisterCleanupFunc(func() error {
		...
		err = client.Steve.SteveType(ProvisioningResouceType).Delete(cluster)
		if err != nil {
